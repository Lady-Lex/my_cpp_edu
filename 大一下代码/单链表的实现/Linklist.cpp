...
public:
	...
	LinkList(T a[], int n);                                         //头插法：有参构造函数,使用含有n个元素的数组a初始化单链表
	LinkList(T a[], double n);                                      //尾插法：有参构造函数,使用含有n个元素的数组a初始化单链表
	~LinkList();                                                    //析构函数
...
template <class T>
LinkList<T>::LinkList(T a[], double n)                              //尾插法建立单链表
{
	front = new Node<T>; 
	auto r = front;                                                 //建立尾指针
	for (int i = 0; i < n; i++)
